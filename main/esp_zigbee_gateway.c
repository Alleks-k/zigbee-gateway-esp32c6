...
// Глобальні змінні статусу Zigbee (без static для зовнішнього доступу)
uint16_t pan_id = 0;
uint8_t channel = 0;
...
void esp_zb_app_signal_handler(esp_zb_app_signal_t *signal_struct)
{
    ...
    case ESP_ZB_BDB_SIGNAL_DEVICE_REBOOT:
        if (err_status == ESP_OK) {
            ...
            if (esp_zb_bdb_is_factory_new()) {
                ...
                // Оновлення статусу для існуючої мережі
    ...
    case ESP_ZB_ZDO_SIGNAL_DEVICE_ANNCE:
        ...
        add_device(dev_annce_params->device_short_addr);
        break;
...
void app_main(void)
{
    ...
    // Ініціалізація SPIFFS для веб-файлів
